...
public class PlayerMovement : MonoBehaviour
{
    ...
    public bool m_isGrounded { get; set; } = true;
    ...
    void FixedUpdate()
    {
        ...
        if (m_rb.velocity.x != 0)
            m_sprite.flipX = m_rb.velocity.x > 0;
    }
    ...
    bool ScanForGround()
    {
        ...
        var btmLeftEdgeOfPlayer = new Vector2(m_col.bounds.min.x + 0.1f, m_col.bounds.min.y);
        var btmRightEdgeOfPlayer = new Vector2(m_col.bounds.max.x - 0.1f, m_col.bounds.min.y);
    ...
    bool ScanForRightWall()
    {
        // +- 0.1f to bring it in from the edges a bit, the down-raycasts would register on walls when
        // moving towards it otherwise
        var rightTopEdgeOfPlayer = new Vector2(m_col.bounds.max.x, m_col.bounds.max.y - 0.1f);
        var rightBtmEdgeOfPlayer = new Vector2(m_col.bounds.max.x, m_col.bounds.min.y + 0.1f);
    ...
    bool ScanForLeftWall()
    {
        // +- 0.1f to bring it in from the edges a bit, the down-raycasts would register on walls when
        // moving towards it otherwise
        var leftTopEdgeOfPlayer = new Vector2(m_col.bounds.min.x, m_col.bounds.max.y - 0.1f);
        var leftBtmEdgeOfPlayer = new Vector2(m_col.bounds.min.x, m_col.bounds.min.y + 0.1f);
